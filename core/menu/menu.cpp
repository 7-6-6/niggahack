...
c_menu menu;
IDirect3DTexture9* pane = nullptr;
#define UNLEN 256
...
void __stdcall c_menu::setup_resent(IDirect3DDevice9* device) {
	...
	colors[ImGuiCol_WindowBg] = ImVec4(31 / 255.f, 31 / 255.f, 33 / 255.f, 1.0f);
	colors[ImGuiCol_PopupBg] = ImVec4(0.08f, 0.08f, 0.08f, 0.94f);
	colors[ImGuiCol_Border] = ImVec4(20 / 255.f, 20 / 255.f, 20 / 255.f, 1.0f);
	colors[ImGuiCol_BorderShadow] = ImVec4(31 / 255.f, 31 / 255.f, 33 / 255.f, 1.0f);
	colors[ImGuiCol_ChildWindowBg] = ImVec4(31 / 255.f, 31 / 255.f, 33 / 255.f, 1.0f);
	...
	colors[ImGuiCol_Header] = ImVec4(96 / 255.f, 160 / 255.f, 220 / 255.f, 1.0f); //multicombo, combo selected item color.
	...
	colors[ImGuiCol_Separator] = ImVec4(31 / 255.f, 31 / 255.f, 33 / 255.f, 1.0f);
	...
	if (pane == nullptr)
	{
		D3DXCreateTextureFromFileInMemoryEx(device, &neitsytuh, sizeof(neitsytuh), 870, 600, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &pane);
	}
...
void c_menu::run() {
	...
	if (opened) {
		ImGui::GetStyle().Colors[ImGuiCol_CheckMark] = ImVec4(149 / 255.f, 184 / 255.f, 6 / 255.f, 1.f);
		...
		ImGui::Begin("nigga hack", &opened, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar); {
			ImVec2 p = ImGui::GetCursorScreenPos();
			...
			ImGui::GetWindowDrawList()->AddImage(pane, ImVec2(ImGui::GetWindowPos().x, ImGui::GetWindowPos().y), ImVec2(ImGui::GetWindowPos().x + 870, ImGui::GetWindowPos().y + 600), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255));
			ImGui::PushFont(font_menu);
			ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 7);
			ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 7);
			ImGui::Text("nigga hack");
			ImGui::SameLine();

			ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 9);
			if (ImGui::ButtonT("aimbot", ImVec2(40, 30), page, 0, false, false)) page = 0; ImGui::SameLine(0, 0);
			...
			if (ImGui::ButtonT("MM", ImVec2(40, 30), page, 5, false, false)) page = 5; ImGui::SameLine(0, 0);
			ImGui::PopFont();
			...
			case 0:
				...
				ImGui::BeginChild("aimbot", ImVec2(279, 268), true); {
					...
					ImGui::Combo("mode", &config_system.item.aim_mode, "adaptive\0HS-only"); //todo add custom bone selection - designer
					ImGui::Checkbox("dynamic fov", &config_system.item.aim_distance_based_fov);
					ImGui::Checkbox("silent", &config_system.item.aim_silent);
					ImGui::Checkbox("smoke check", &config_system.item.smoke_check);
					ImGui::Checkbox("target teammates", &config_system.item.aim_team_check);
				...
				ImGui::BeginChild("backtrack", ImVec2(279, 267), true); {
					ImGui::Checkbox("enable", &config_system.item.backtrack);
					ImGui::PushStyleColor(ImGuiCol_Text, config_system.item.backtrack ? ImVec4(1.f, 1.f, 1.f, 1) : ImVec4(.6f, .6f, .6f, 1));
					ImGui::SliderFloat("records (ms)", &config_system.item.backtrack_ms, 1.0f, 250.f, "%.2f");
					ImGui::PopStyleColor();
				}
				ImGui::EndChild(true);
			...
			case 1:
				...
				ImGui::BeginChild("player", ImVec2(279, 268), true); {
					...
					if (config_system.item.visuals_enabled) {
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 7);
						ImGui::Checkbox("teammates", &config_system.item.visuals_team_check);
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 7);
					...
					if (ImGui::BeginCombo("flags", "...", ImVec2(0, 105))) {
						...
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
						ImGui::Selectable(("tickbase"), &config_system.item.player_flags_nigga2, ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
						...
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
						ImGui::Selectable(("flash duration"), &config_system.item.player_flags_nigga, ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
				...
				ImGui::BeginChild("effects", ImVec2(279, 267), true); {
					...
					ImGui::Checkbox("grenade prediction", &config_system.item.grenadepred);
				...
				ImGui::BeginChild("chams", ImVec2(279, 267), true); {
					...
					ImGui::Checkbox("sleeves", &config_system.item.sleevechams);
					ImGui::ColorEdit4("sleeves color", config_system.item.sleevescolor, ImGuiColorEditFlags_NoInputs);
					ImGui::Combo("sleeve type", &config_system.item.sleevetype, "textured\0flat\0metallic\0pulsating");
				

					//fix it ur self
					/*ImGui::Checkbox("hands", &config_system.item.handchams);
					ImGui::ColorEdit4("hands color", config_system.item.handscolor, ImGuiColorEditFlags_NoInputs);
					ImGui::Combo("hand type", &config_system.item.armtype, "textured\0flat\0metallic\0pulsating");

					ImGui::Checkbox("weapon", &config_system.item.weaponchams);
					ImGui::ColorEdit4("weapon color", config_system.item.weaponchamscolor, ImGuiColorEditFlags_NoInputs);
					ImGui::Combo("weapon type", &config_system.item.weaponchamcombo, "textured\0flat\0metallic\0pulsating");*/
			...
			case 2:
				...
				ImGui::BeginChild("misc", ImVec2(279, 543), true); {
					...
					if (ImGui::BeginCombo("logs", "...", ImVec2(0, 65))) {
						...
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
						ImGui::Selectable(("extra hurt info"), &config_system.item.extra_info, ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
						...
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
						ImGui::Selectable(("bomb plant"), &config_system.item.bombplant, ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
						ImGui::SetCursorPosX(ImGui::GetCursorPosX() - 4);
				...
				ImGui::BeginChild("movement", ImVec2(279, 543), true); {
					...
					ImGui::Checkbox("auto strafe", &config_system.item.strafe);
					...
					if (config_system.item.edge_jump) {
						ImGui::Hotkey("##edgejump", &config_system.item.edge_jump_key, ImVec2(100, 20));
			...
			case 3:
				...
				ImGui::BeginChild("weapons", ImVec2(279, 543), true); {
					...
					case 0:
						...
						if (ImGui::Button("apply", ImVec2(84, 18))) {
			...
			case 5:
				...
				ImGui::PushStyleColor(ImGuiCol_ChildWindowBg, ImVec4(30 / 255.f, 30 / 255.f, 39 / 255.f, 1.0f));
				ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f));
				ImGui::BeginChild("grief", ImVec2(279, 268), true); {
					ImGui::Checkbox("door spam", &config_system.item.doorspam_enabled);
					if (config_system.item.doorspam_enabled) {
						ImGui::Hotkey("##doorspam key", &config_system.item.doorspam_key, ImVec2(100, 20));
					}
					ImGui::Checkbox("name spam", &config_system.item.name_spam);
					ImGui::Checkbox("blockbot", &config_system.item.blockbot);
					if (config_system.item.blockbot) {
						ImGui::Hotkey("##blockbot key", &config_system.item.blockbot_key, ImVec2(100, 20));
					}
				}
				ImGui::EndChild(true);
				...
				ImGui::BeginChild("exploits", ImVec2(279, 268), true); {
					if (ImGui::Button("hide name", ImVec2(84, 20))) {
						utilities::change_name("\n\xAD\xAD\xAD");
					}
					if (ImGui::Button("rainbow name", ImVec2(84, 20))) {
						utilities::change_name("\n\xAD\xAD\xAD \x01\x0B\x02""N""\x04""I""\x010""G""\x0E""G""\x09""A ""\x05""H""\x02""A""\x05""C""\x02""K""\x05""0""\x02""5""\x05""7""\x02""6""\x05""6""\x03");
					}
					if (ImGui::Button("vote troll", ImVec2(84, 20))) {
						utilities::change_name("\n\xAD\xAD\xAD\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nnigga\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
					}
					if (ImGui::Button("make lobby prime", ImVec2(120, 20)))
					{
						utilities::make_lobby_prime();
					}
					ImGui::Checkbox("nigga tag", &config_system.item.niggatag);
				...
				ImGui::NextColumn();
				ImGui::BeginChild("general", ImVec2(279, 543), true); {
					ImGui::Checkbox("enable", &config_system.item.grief_enable);
					if (ImGui::Button("dump steam id", ImVec2(84, 20))) {
