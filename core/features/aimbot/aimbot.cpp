...
void c_aimbot::run(c_usercmd* user_cmd) noexcept {
	...
	if (config_system.item.aim_enabled && user_cmd->buttons & in_attack) {
		if (auto target = find_target(user_cmd)) {
			...
			case 0:
				...
				break;
			case 1:
				angle = math.calculate_angle(local_player->get_eye_pos(), entity->get_hitbox_position(entity, hitbox_head), user_cmd->viewangles);
				break;
			}
