...
void c_hooked_events::fire_game_event(i_game_event* event) noexcept {
	...
	else if (strstr(event->get_name(), "bomb_beginplant"))
	{
		event_logs.bombplant(event);
	}
	...
	else if (!strcmp(event_name, "player_connect")) {
		event_logs.connect(event);
	}
	else if (!strcmp(event_name, "player_disconnect")) {
		event_logs.disconnect(event);
	}
}
...
void c_hooked_events::setup() noexcept {
	...
	interfaces::event_manager->add_listener(this, "player_connect", false);
	interfaces::event_manager->add_listener(this, "player_disconnect", false);
}
