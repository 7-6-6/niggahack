...
void c_visuals::grenadepred() noexcept
{
	if (!config_system.item.visuals_enabled)
		return;

	if (config_system.item.grenadepred) {
...
void c_visuals::player_rendering(player_t* entity) noexcept {
	if ((entity->dormant() && alpha[entity->index()] == 0))
		return;
	...
	}
	{
		...
		if (config_system.item.player_flags_nigga && entity->is_flashed())
			flags.push_back(std::pair<std::string, color>(std::string("flash duration: ").append(std::to_string(entity->flash_duration())), color(255, 255, 255, alpha[entity->index()])));

		if (config_system.item.player_flags_nigga2)
			flags.push_back(std::pair<std::string, color>(std::string("tickbase: ").append(std::to_string(entity->get_tick_base())), color(255, 255, 255, alpha[entity->index()])));
...
void c_visuals::chams() noexcept {
	...
	for (int i = 1; i <= interfaces::globals->max_clients; i++) {
		...
		auto textured = interfaces::material_system->find_material("nigga hack_material", TEXTURE_GROUP_MODEL, true, nullptr);
		auto metalic = interfaces::material_system->find_material("nigga hack_reflective", TEXTURE_GROUP_MODEL, true, nullptr);
...
void c_visuals::viewmodel_modulate(const model_render_info_t& info) {
	...
	static i_material* nigga = nullptr;
	auto textured = interfaces::material_system->find_material("nigga hack_material", TEXTURE_GROUP_MODEL, true, nullptr);
	auto metalic = interfaces::material_system->find_material("nigga hack_reflective", TEXTURE_GROUP_MODEL, true, nullptr);
	auto dogtag = interfaces::material_system->find_material("models/inventory_items/dogtags/dogtags_outline", TEXTURE_GROUP_MODEL, true, nullptr);
	auto flat = interfaces::material_system->find_material("debug/debugdrawflat", TEXTURE_GROUP_MODEL, true, nullptr);
	...
	static i_material* nigga2 = nullptr;
	auto textured2 = interfaces::material_system->find_material("nigga hack_material", TEXTURE_GROUP_MODEL, true, nullptr);
	auto metalic2 = interfaces::material_system->find_material("nigga hack_reflective", TEXTURE_GROUP_MODEL, true, nullptr);
	auto dogtag2 = interfaces::material_system->find_material("models/inventory_items/dogtags/dogtags_outline", TEXTURE_GROUP_MODEL, true, nullptr);
	auto flat2 = interfaces::material_system->find_material("debug/debugdrawflat", TEXTURE_GROUP_MODEL, true, nullptr);
	textured2->increment_reference_count();  //we need increment_reference_count cuz without it our materialsystem.dll will crash after  map change - designer
	metalic2->increment_reference_count();
	flat2->increment_reference_count();
	dogtag2->increment_reference_count();

	static i_material* nigga3 = nullptr;
	auto textured3 = interfaces::material_system->find_material("nigga hack_material", TEXTURE_GROUP_MODEL, true, nullptr);
	auto metalic3 = interfaces::material_system->find_material("nigga hack_reflective", TEXTURE_GROUP_MODEL, true, nullptr);
	auto dogtag3 = interfaces::material_system->find_material("models/inventory_items/dogtags/dogtags_outline", TEXTURE_GROUP_MODEL, true, nullptr);
	auto flat3 = interfaces::material_system->find_material("debug/debugdrawflat", TEXTURE_GROUP_MODEL, true, nullptr);
	textured3->increment_reference_count();  //we need increment_reference_count cuz without it our materialsystem.dll will crash after  map change - designer
	metalic3->increment_reference_count();
	flat3->increment_reference_count();
	dogtag3->increment_reference_count();

	switch (config_system.item.sleevetype) {
	case 0:
		nigga = textured;
		break;
	case 1:
	...
	switch (config_system.item.armtype) {
	case 0:
		nigga2 = textured2;
		break;
	case 1:
	...
	switch (config_system.item.weaponchamcombo) {
	case 0:
		nigga3 = textured3;
		break;
	case 1:
	...
	if (strstr(model_name, "sleeve")) {
		...
		if (config_system.item.sleevechams)
		{
			interfaces::render_view->modulate_color(config_system.item.sleevescolor);
			interfaces::render_view->set_blend(config_system.item.sleevescolor[3]);

			interfaces::model_render->override_material(nigga);
		}
	}
	if (strstr(model_name, "arms"))
	{
		if (config_system.item.handchams)
		{
			interfaces::render_view->modulate_color(config_system.item.handscolor);
			interfaces::render_view->set_blend(config_system.item.handscolor[3]);

			interfaces::model_render->override_material(nigga2);
		}
	}	
	//fix it ur self, sleeve chams work rest don't
	if (strstr(model_name, "models/weapons/v_") || (strstr(model_name,"v_")))
	{
		if (config_system.item.weaponchams)
		{
			interfaces::render_view->modulate_color(config_system.item.weaponchamscolor);
			interfaces::render_view->set_blend(config_system.item.weaponchamscolor[3]);

			interfaces::model_render->override_material(nigga3);
		}
	}
	if (strstr(model_name, "arms")) {
