...
void utilities::make_lobby_prime()
{
	if (interfaces::engine->is_connected() || interfaces::engine->is_in_game()) {
		utilities::console_warning("[nigga hack] ");
		interfaces::console->console_printf("you must be in a lobby to make it prime.\n");
		return;
	}

	//lobby shit
	using Fn = void(__fastcall*)(const char*);
	static auto prime = reinterpret_cast<Fn>(utilities::pattern_scan(GetModuleHandleA("client_panorama.dll"), "A1 ? ? ? ? 85 C0 75 07 83 F8 05 0F 94 C0 C3"));
	DWORD old_protect;
...
void utilities::material_setup() {
	std::ofstream("csgo/materials/nigga hack_material.vmt") << R"#("VertexLitGeneric" {
	...
	std::ofstream("csgo/materials/nigga hack_reflective.vmt") << R"#("VertexLitGeneric" {
			"$basetexture" "vgui/white_additive"
			"$ignorez" "0"
			"$envmap" "env_cubemap"
